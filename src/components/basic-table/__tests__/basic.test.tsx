...
describe('Basic Tabel Testing', () => {
  ...
  it('排序功能 commonSorter 正常', async () => {
    ...
    expect(getByText('Age：升序')).toBeVisible();
    ...
    // 降序
    fireEvent.click(element);
    await waitForElement(() => getByText('Age：降序'));
    expect(getByText('Age：降序')).toBeVisible();
  ...
  it('过滤功能 commonFilter 正常', async () => {
    ...
    const okBtn = await waitForElement(() => getByText('确定'));
    fireEvent.click(okBtn);
    await waitForElement(() => getByText('共 1 条'));

    // 重置
    const clearBtn = await waitForElement(() => getByTestId('btn-clearall'));
    fireEvent.click(clearBtn);
    await waitForElement(() => getByText('共 3 条'));
  });
